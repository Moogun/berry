...
class XAQuery:
    ...
    def requestService(self, block=True):
        print('requestService')
        self.com_obj.RequestService("t1857", "") # the second arg is where the live data key is to be added
        if block:
            while not self.received:
                pythoncom.PumpWaitingMessages()
    ...
    # 주의
    # outblock 으로 시작하는 경우도  ------ t1857은 outblock 으로 시작
    # outblock1 으로 시작하는 경우도 있다.
    ...
    def block_request(self, *args, **kwargs):
        ...
        for outblock in res_data['outblock']:
            ...
            for i in range(rows):
                ...
                if "AlertNum" in elem.keys():
                    print('AlertNum --', elem['AlertNum'])

                elif "cts_time" in elem.keys():
                    print('cts_time --', elem['cts_time'])

                data.append(elem)

            # below assignment to df overwrites the previous data (block)
            df = pandas.DataFrame(data=data)
            ret.append(df)
        return ret, cts_time_received
